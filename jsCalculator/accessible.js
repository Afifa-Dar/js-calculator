...
var inputKey = document.querySelectorAll('input')
...
const doCalc = (op1 , op2 , calc) => {
     res = calc(op1 , op2)
console.log(op1,op2,calc,typeof op1 ,typeof op2)
    if(res == "Cant divide by zero"){   //user try to div number by 0
        alert("Cant divide by zero")
        clearHistory() 
        //input.value=res       //show error
        input.value =""   
        flag = -1
    ...
    {
        res = res + input.value.slice(-1)   //show in inputBar
    }

    input.value = res      //show computed value to input bar
    ...
    if (btn.innerHTML !== clear.innerHTML && btn.innerHTML !== cut.innerHTML && btn.innerHTML !== equal.innerHTML)
     {
        if(flag !== undefined)  // if history was clear
         {
            if(btn.value in operator)   //if  user press operator once history clear
            {
                ...
                input.value=btn.value  //initilize inputbar to  input
           }
        ...
        input.value += btn.innerText   //addd value to input
    }  
...
//any keyPressed
inputKey.forEach(inp => inp.addEventListener("keyup", (event) => {
    // if (!(event.key in num || event.key in oper || event.keyCode == 13 || event.keyCode == 8 || event.keyCode == 61)){
    //     console.log('undefined operand')
    //     alert('undefined operation')
    //     input.value = input.value = ""
    //     clearHistory()
    //     flag == -1
    // }
    if(event.value in num || event.value in oper || event.value === '=' || event.keyCode==13 || event.keyCode == 8 ){
        console.log('undefined operand')
        alert('undefined operation')
         input.value = input.value = ""
        clearHistory()
